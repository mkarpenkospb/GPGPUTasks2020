...
{    
    ...
    as_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), as.data(), &err);
    OCL_SAFE_CALL(err);

    bs_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), bs.data(), &err);
    OCL_SAFE_CALL(err);

    cs_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, n * sizeof(float), cs.data(), &err);
    OCL_SAFE_CALL(err);
...
//        std::ifstream file("../src/cl/aplusb.cl");
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &strings, nullptr, &err);
    OCL_SAFE_CALL(err);
    ...
    cl_kernel kernel;
    kernel = clCreateKernel(program, "aplusb", &err);
    OCL_SAFE_CALL(err);
...

    {
        ...
        std::cout << "GFlops: " << (n * 1.0 / 1000*1000*1000) / t.lapAvg()  << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << (n*sizeof(float) * 3.0 / (1024*1024*1024)) / t.lapAvg() << " GB/s" << std::endl;
    }
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, cs_gpu, CL_TRUE, 0, n, cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << (n*sizeof(float) * 1.0 / (1024*1024*1024)) / t.lapAvg() << " GB/s" << std::endl;
    }
    ...
    for (unsigned int i = 0; i < n; ++i) {
        // они расходятся и так, почему не сранивается с епсилон?
        if (cs[i] != as[i] + bs[i]) {
            std::stringstream ss;
            ss << "CPU and GPU results differ with CPU: " <<  std::fixed << std::setprecision(10) <<
            as[i] + bs[i] << " vs GPU: " << cs[i];
